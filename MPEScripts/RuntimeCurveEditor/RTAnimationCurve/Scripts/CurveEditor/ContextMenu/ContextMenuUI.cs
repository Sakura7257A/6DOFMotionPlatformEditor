...
namespace RuntimeCurveEditor
{
    ...
    public class ContextMenuUI : MonoBehaviour
    {
        ...
        public RectTransform weightedTangent;
        ...
        RectTransform checkMarkWeighted;
        ...
        void Start() {
            ...
            tangentTypesMenuItems = new List<RectTransform>() { freeTangent, linearTangent, constantTangent, weightedTangent };
                ...
                if (panel.gameObject.activeSelf)
                {
                    ...
                    else
                    {
                        ...
                        if (tangentMenuItemHovered != null)
                        {
                            ...
                            if (((tangentMenuItemHovered == leftTangent) && contextMenu.leftTangent.weighted) ||
                                    ((tangentMenuItemHovered == rightTangent) && contextMenu.rightTangent.weighted) ||
                                    ((tangentMenuItemHovered == bothTangents) && contextMenu.bothTangents.weighted))
                            {
                                if (checkMarkWeighted == null)
                                {
                                    checkMarkWeighted = Instantiate(checkMark.gameObject).GetComponent<RectTransform>();
                                    checkMarkWeighted.SetParent(panelTangent);
                                    checkMarkWeighted.localPosition = new Vector2(-panelTangent.sizeDelta.x * 0.5f + checkMark.sizeDelta.x, 0);
                                }
                                checkMarkWeighted.gameObject.SetActive(true);
                                checkMarkWeighted.localPosition = new Vector2(checkMarkWeighted.localPosition.x, weightedTangent.localPosition.y);
                            }
                            else if (checkMarkWeighted != null)
                            {
                                checkMarkWeighted.gameObject.SetActive(false);
                            }
                        }
                        else
                        {
        ...
        void TriggerListnerer(RectTransform selectedItem) {
            ...
            } else if (weightedTangent == selectedItem) {
                interfaceContextMenuListener.Weighted(GetTangentPart(), true);
            }
        }
