            ...
            if (keyframeWorld.x > point.x) {
                ...
                if (keyframeWorld.x < point.x) {
                    ...
                    for (int i = 0; i < animCurve.length - 1; ++i) {
                        var keyOut = animCurve[i];
                        var keyIn = animCurve[i + 1];
                        keyframeWorld = Convert(new Vector2(keyIn.time, keyIn.value), gridRect, curveGradRect);
                        if (point.x < keyframeWorld.x) {
                            Vector2 p1 = new Vector2(keyOut.time, keyOut.value);
                            ...
                            if (contextMenuManager.dictCurvesContextMenus[animCurve][i].rightTangent.constant ||
                               ...
                               keyOut.outTangent == float.PositiveInfinity || keyIn.inTangent == float.PositiveInfinity) {
                                ...
                                Curves.GetControlPoints(p1, p2, keyOut.outTangent * ratio, keyIn.inTangent * ratio, out c1, out c2, tangWeightOut, tangWeightIn);
