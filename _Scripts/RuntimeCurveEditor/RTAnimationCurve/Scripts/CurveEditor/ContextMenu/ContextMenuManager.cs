...
namespace RuntimeCurveEditor
{
    ...
    {
        public class ReferenceAnimationCurveEqualityComparer : IEqualityComparer<AnimationCurve>
        {
            private ReferenceAnimationCurveEqualityComparer() { }

            public static readonly ReferenceAnimationCurveEqualityComparer Instance = new ReferenceAnimationCurveEqualityComparer();

            public bool Equals(AnimationCurve curve1, AnimationCurve curve2)
            {
                return ReferenceEquals(curve1, curve2);
            }

            public int GetHashCode(AnimationCurve curve)
            {
                return RuntimeHelpers.GetHashCode(curve);
            }
        }
        ...
        /// </summary>	
        public Dictionary<AnimationCurve, List<ContextMenu>> dictCurvesContextMenus = new Dictionary<AnimationCurve, List<ContextMenu>>(ReferenceAnimationCurveEqualityComparer.Instance);
        ...
        public static bool IsKeyframeFreeSmooth(Keyframe keyframe)
        {
            return Mathf.Abs(keyframe.inTangent - keyframe.outTangent) < Mathf.Epsilon;
        }
        ...
        public void AddContextMenuObjects(AnimationCurve curve) {
            if (curve != null) {
                ...
                foreach (Keyframe keyframe in curve.keys) {
                    ...
                    if (IsKeyframeFreeSmooth(keyframe)) {
